...
interface MemberAuthFormContainerProps {
  lang: Language;
  type: "login" | "register";
  isLoading: boolean;
  handleSubmit: (data: {
  ...
  handleSocialLogin: (platform: string) => void;
  handleSwitchType: () => void;
}

export function MemberAuthFormContainer({
  lang,
  type,
  isLoading,
  handleSubmit,
...
function MemberAuthForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!email) newErrors.email = t.enterEmail;
    else if (!/\S+@\S+\.\S+/.test(email)) newErrors.email = t.invalidEmail;

    if (!password) newErrors.password = t.enterPassword;
    else if (password.length < 8) newErrors.password = t.passwordMinLength;

    if (type === "register" && !username) newErrors.username = t.enterUsername;
    ...
    onSubmit({
      email,
      password,
      name: type === "register" ? username : undefined,
    });
  };
    ...
    <div className="w-full max-w-md">
      <div className="text-center mb-10">
        ...
        <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">
          {type === "login" ? t.welcomeBack : t.startJourney}
        </h2>
        <p className="text-slate-500 dark:text-slate-400">
          {type === "login" ? t.loginDesc : t.registerDesc}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-[2rem] shadow-2xl">
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  } rounded-xl py-3.5 pl-12 pr-4 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all`}
                  placeholder={t.enterUsername}
                />
              </div>
              {errors.username && (
                <p className="mt-1.5 text-xs text-rose-500 font-medium ml-1">
                  {errors.username}
                </p>
              )}
            </div>
              ...
              <input
                ...
                className={`w-full bg-slate-50 dark:bg-slate-800/50 border ${
                  errors.password
                    ? "border-rose-500"
                    : "border-slate-200 dark:border-slate-700"
                } rounded-xl py-3.5 pl-12 pr-12 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all`}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl transition-all shadow-xl shadow-blue-600/20 flex items-center justify-center gap-2 group active:scale-[0.98]"
          >
            {isLoading ? (
              <Loader2 className="w-5 h-5 animate-spin" />
            ) : (
              <>
                <span>{type === "login" ? t.login : t.registerNow}</span>
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 text-center">
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            ...
            >
              {type === "login" ? t.createAccount : t.loginNow}
            </button>
          </p>
        </div>
      </div>
