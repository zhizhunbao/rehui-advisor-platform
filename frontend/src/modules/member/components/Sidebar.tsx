...
import { memberLocales } from "../locales";
        ...
        {user ? (
          isCollapsed ? (
            ...
            <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-200/70 dark:hover:bg-white/10 transition-colors cursor-pointer">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white text-sm font-medium shrink-0">
                {displayInitial}
              </div>
              <div className="flex-1 min-w-0">
                ...
                >
                  {(quotaStatus?.remaining ?? 0) <= 3 &&
                  (quotaStatus?.remaining ?? 0) > 0
                    ? `⚠️ ${t.quotaLeft}: `
                    : `${t.quotaLeft}: `}
                  {quotaStatus?.remaining ?? 0}/{quotaStatus?.searchLimit ?? 20}
                </div>
              </div>
              <button
                onClick={onLogout}
