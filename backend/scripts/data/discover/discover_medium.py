# Medium 资源发现脚本 - 通过标签搜索 Medium 文章
import os
import time
...
class DiscoverMediumScript(ScriptBase):
    """通过标签搜索 Medium 发现新资源"""

    NAME = "discover_medium"
    DESCRIPTION = "通过标签搜索 Medium 文章"

    MEDIUM_URL = "https://medium.com"
    OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "data_source", "medium", "__init__.py")
    ...
    def init_file(self) -> None:
        ...
        with open(self.OUTPUT_FILE, "w", encoding="utf-8") as f:
            f.write("# Medium 数据源 - 由 discover_medium.py 脚本自动生成\n")
            f.write("from typing import List, Dict, Any\n\n")
            f.write("MEDIUM_SOURCES: List[Dict[str, Any]] = [\n")
    ...
    def close_file(self) -> None:
        ...
        with open(self.OUTPUT_FILE, "a", encoding="utf-8") as f:
            f.write("]\n\n")
            f.write('__all__ = ["MEDIUM_SOURCES"]\n')
    ...
    def search_articles(self, tag: str) -> List[Dict[str, Any]]:
        """搜索 Medium 文章（通过标签页面）"""
        tag_url = f"{self.MEDIUM_URL}/tag/{tag.lower().replace(' ', '-')}"
        try:
            resp = requests.get(tag_url, timeout=30)
            if resp.status_code == 200:
                return [{"tag_url": tag_url, "tag": tag}]
            return []
        except requests.RequestException as e:
            self.error(f"搜索失败: {tag} - {e}")
            return []

    def discover_by_domain(self, domain_code: str) -> List[Dict[str, Any]]:
        """通过领域发现资源"""
        ...
        for tag_code, tag_name in tags:
            ...
            self.info(f"搜索 [{tag_info['category']}] > [{tag_info['domain']}] > {tag_slug}")
            results.append({
                "url": f"{self.MEDIUM_URL}/tag/{tag_slug}",
                "name": f"Medium - {tag_name}",
                "description": f"Medium {tag_name} 相关文章",
                "source_type": "medium",
    ...
    def discover_all(self, domain_codes: List[str] | None = None) -> int:
        """发现所有领域的资源"""
        ...
        for i, domain_code in enumerate(domain_codes):
            ...
            results = self.discover_by_domain(domain_code)
            for item in results:
...
if __name__ == "__main__":
    script = DiscoverMediumScript(verbose=True)
    result = script.run()
    print(result)
